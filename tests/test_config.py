...
def test_default_config_file_is_valid(tmp_path: Path) -> None:
    ...
    assert config.state_retention_days == 90
...
def test_retention_below_anaf_window_is_rejected() -> None:
    with pytest.raises(ValueError, match="state_retention_days"):
        SyncConfig.model_validate({"cif": "12345678", "state_retention_days": 59})
